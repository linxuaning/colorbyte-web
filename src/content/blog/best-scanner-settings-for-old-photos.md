...
description: "Exact scanner settings for old photos by type: prints, 35mm negatives, slides, and damaged originals. Resolution, color depth, format, and what AI can recover from each."
...
faq:
  ...
  - question: "Should I scan in 24-bit or 48-bit color depth?"
    answer: "Scan damaged or faded photographs in 48-bit color depth (16 bits per channel). The extra bit depth captures tonal gradations in highlight and shadow regions that 24-bit scanning clips or quantizes, and this information becomes critical when you process the scan through restoration software or AI tools. For a severely faded print where the entire image is in the upper quarter of the tonal range, 24-bit scanning collapses all those pale tones into a narrow range that loses detail — 48-bit preserves the distinctions. For well-preserved prints with full tonal range, 24-bit is sufficient and produces smaller files. The practical rule is: if the photograph looks pale, flat, or has visible color shifts (yellowing, reddening, or greening from dye fading), scan at 48-bit. File sizes at 48-bit are double the equivalent 24-bit scan, so storage planning matters for large batch projects. AI restoration tools like those on ArtImageHub accept standard 24-bit JPEG or PNG input — convert 48-bit TIFF files after restoration editing."
  ...
  - question: "How does scanning resolution affect what AI restoration can recover?"
    answer: "AI restoration models work with existing pixel information — they cannot invent detail that was never captured in the scan. Real-ESRGAN, the upscaling model used in ArtImageHub's restoration pipeline, uses learned patterns from millions of photographs to intelligently extrapolate fine detail, but its output quality is directly bounded by the input resolution. A 300 DPI scan of a damaged print gives Real-ESRGAN limited raw material, and the upscaled result will show visible interpolation artifacts in fine-detail areas like hair, fabric textures, and handwritten text. A 1200 DPI scan of the same print gives the model four times as much information per inch, and the upscaled output is correspondingly sharper and more accurate in detail reconstruction. The practical implication: if you plan to run AI restoration after scanning, always scan at the higher end of the appropriate resolution range for your photo type. The cost is larger files; the benefit is measurably better restoration output. For faces specifically, higher scan resolution almost always produces noticeably better GFPGAN face restoration results."
---
