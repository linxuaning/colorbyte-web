...
description: "Restore photos for Ancestry, FamilySearch, and MyHeritage. Upload formats, size limits, EXIF metadata embedding, and how AI restoration improves face matching."
...
faq:
  - question: "What file format and size should I use when uploading restored photos to Ancestry.com?"
    answer: "Ancestry.com accepts JPEG, PNG, GIF, BMP, and TIFF files for photo uploads. JPEG is the recommended format for restored photographs: it produces smaller file sizes without visible quality loss at 90-95% quality settings and loads faster in Ancestry's photo viewer. The maximum file size on Ancestry is 25MB per image — well above what a restored JPEG typically occupies (3-8MB for a 1200 DPI scan of a 4x6 print). Photos are resized to approximately 1200-1600 pixels wide for display, but the full-resolution original is preserved and available for download by tree collaborators. Upload the full-resolution AI-restored output so collaborators have the highest-quality version. Avoid uploading TIFFs directly — technically supported but slow to load and poorly handled by the Ancestry mobile app. For FamilySearch, TIFF is preferred for long-term preservation; for MyHeritage, JPEG only (TIFF not accepted by the standard uploader)."
  - question: "How does AI photo restoration improve face recognition in genealogy databases?"
    answer: "Genealogy platforms including MyHeritage and Ancestry use face detection and recognition algorithms to suggest matches between photographs across family trees. These algorithms detect facial landmark points — eye corners, nose bridge, jawline — and compare them against a database of detected faces. Degraded photographs create a direct problem: low resolution, blur, noise, and damage artifacts reduce the number of landmarks the detection algorithm can locate reliably, producing false non-matches or complete detection failure. AI restoration addresses each degradation mode. GFPGAN, the face restoration model in ArtImageHub's pipeline, reconstructs facial detail — eyes, skin texture, hair edges — that face detection algorithms rely on. Real-ESRGAN upscaling increases pixel count in facial regions, giving the detection algorithm more data. AI-restored photographs consistently produce more face detection hits and higher confidence scores than degraded originals. Restoring a portrait before uploading to MyHeritage directly improves genealogical matching effectiveness."
  - question: "What EXIF metadata should I embed in genealogy photos before uploading?"
    answer: "Embedding metadata before uploading creates a permanent, machine-readable record of what the photograph documents. The most important EXIF fields to complete are: DateTimeOriginal (estimated capture date in YYYY:MM:DD format — use 1942:00:00 if only the year is known); ImageDescription (names of people pictured, relationship, location, and occasion); Artist or Copyright (the photograph owner's name); and GPSLatitude/GPSLongitude (approximate location — town center coordinates are sufficient precision). Use ExifTool (free, command-line) or the metadata editor in Adobe Bridge to write these fields before upload. Most genealogy platforms read ImageDescription and display it as the photo caption, saving manual re-entry time in the web interface. Platform behavior varies: Ancestry shows description and date; FamilySearch shows all embedded fields in its photo detail view; MyHeritage parses date and description. Always embed before uploading — most platforms strip or ignore metadata added after the initial upload."
  ...
  - question: "What are the privacy considerations for uploading restored family photos to genealogy websites?"
    answer: "The main privacy concern on genealogy platforms is photographs of living individuals. Ancestry, FamilySearch, and MyHeritage permit uploading photos of living people to private family trees with controlled visibility, but public tree settings expose those photos to all users and sometimes to search engine indexing. Best practice: set the relevant section to private or tag individuals as living (Ancestry auto-hides living individuals' details from public view). For photographs of only deceased individuals — nearly all old photo restoration use cases — public tree settings are appropriate. An important nuance: AI-restored photographs are significantly clearer than originals, which can make previously unidentifiable background individuals identifiable. If a restored group photo reveals faces not visible in the original, verify whether any of those individuals may still be living before publishing. MyHeritage's face recognition runs on all uploaded photos regardless of privacy setting — to prevent face-matching, keep the photo in a private tree section entirely."
---
