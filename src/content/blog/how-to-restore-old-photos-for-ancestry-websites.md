...
description: "Restore old photos for Ancestry, FamilySearch, and MyHeritage. File formats, size limits, EXIF metadata, and how AI face restoration improves genealogy matching results."
...
faq:
  ...
  - question: "What EXIF metadata should I embed in genealogy photos before uploading?"
    answer: "Embedding metadata before uploading creates a permanent, machine-readable record of what the photograph documents. The most important fields to complete are: DateTimeOriginal (estimated capture date in YYYY:MM:DD format — use 1942:00:00 if only the year is known); ImageDescription (names of people pictured, relationship, location, and occasion); Artist or Copyright (the photograph owner's name); and GPSLatitude/GPSLongitude (approximate location — town center coordinates are sufficient). Use ExifTool (free, command-line) or the metadata editor in Adobe Bridge to write these fields before upload. Most genealogy platforms read ImageDescription and display it as the photo caption, saving manual re-entry time. Platform behavior varies: Ancestry shows description and date; FamilySearch shows all embedded fields in its photo detail view; MyHeritage parses date and description. Always embed before uploading — most platforms strip or ignore metadata added after the initial upload event."
