...
description: "How to recover photos from degraded DVDs and CDs — disc rot, CD-R dye oxidation, Kodak Picture CD formats — and remove compression artifacts with NAFNet after recovery."
...
faq:
  - question: "What is DVD rot and how does it damage photos stored on discs?"
    answer: "DVD rot is the colloquial term for disc delamination — a manufacturing or storage failure where the adhesive bonding the reflective aluminum layer to the polycarbonate substrate breaks down, allowing oxygen and moisture to reach the reflective layer and oxidize it. The process is visible as iridescent spots, cloudiness, or areas where the disc becomes transparent when held to light. These delaminated areas cannot reflect the laser during playback, causing read errors or complete failure in the affected sectors. DVDs manufactured before approximately 2005 used adhesive formulations that proved less durable than later standards. Discs stored in high humidity environments, in paper sleeves, or exposed to UV light are at highest risk. The damage is irreversible — disc rot areas with lost reflective material cannot be read by any drive. Recovery depends entirely on which sectors are affected: if damaged areas cover only data sectors but leave the file system directory intact, the disc may still be partially readable and the undamaged files fully recoverable."
  - question: "How can you tell if a CD-R has dye oxidation and is it recoverable?"
    answer: "CD-R dye oxidation is visible as a color change in the disc's data layer — the area between the clear hub ring and the outer edge that appears green, gold, or blue depending on the original dye formulation. A healthy CD-R shows a uniform color in this region. Oxidized areas appear darker, greenish-brown, or purplish (the purple haze effect). The oxidation degrades optical contrast between burned and unburned pits and lands, reducing error correction headroom available to the disc drive. Mild oxidation results in read errors that error correction handles — the disc reads slowly but completely. Severe oxidation produces uncorrectable errors and missing sectors. Recoverability depends on the pattern of oxidation: outer edge concentration is more recoverable because outer tracks may contain only partial file data. Oxidation spread uniformly across the data area is more likely to hit critical sectors, including the file system directory at the inner tracks — which causes complete read failure even when most disc data is physically intact."
  - question: "What is a Kodak Picture CD and what photo format does it use?"
    answer: "Kodak Picture CD was a consumer service offered by Kodak and photo processing labs from 1998 to approximately 2012. When you dropped off a roll of film for processing, you could pay a few extra dollars to receive a CD-ROM with digital scans of your prints alongside the developed prints and negatives. The scans were saved as JPEG files at a resolution of approximately 1024x1536 pixels (1.5 megapixels) using Kodak's proprietary scanning equipment. The CD contained a simple HTML-based viewer application and a folder named Pictures or Images containing the JPEG files with sequential filenames. Kodak Picture CDs used standard CD-ROM format (ISO 9660 file system) and should be readable by any modern computer optical drive — the content is standard JPEG, not a proprietary image format. The quality limitation of Picture CD photos is the 1.5-megapixel scan resolution, which was acceptable for 4x6 prints in 1998 but is low by current standards. AI upscaling via Real-ESRGAN can increase these to 6-8 megapixel equivalents with significant detail recovery. The JPEG compression used by Kodak was moderate quality (approximately 80-85%), so NAFNet artifact removal before upscaling produces noticeably better results than upscaling directly from the compressed original."
  - question: "What should I do if the disc is completely unreadable?"
    answer: "When a disc fails to read in multiple drives, you have two practical options: attempt physical intervention yourself before paying for professional recovery, or go directly to professional data recovery. Home attempts that sometimes succeed: the angle trick (holding the disc at different angles to the drive's laser, achieved by inserting the disc slightly off-axis using a thin card under the edge), disc polishing to remove the top protective lacquer layer that may have hazed (a disc polishing machine or toothpaste-and-cloth method for minor scratches on the non-label side), and trying multiple optical drives with different laser wavelengths and tolerance specs — some drives handle degraded discs better than others. Professional data recovery services that specialize in optical media can attempt laser power adjustment, specialized reflectivity compensation, and, in extreme cases, delaminated disc reconstruction. Costs range from $50 to $300 for optical media recovery depending on the service and disc condition. The value calculation is personal: if the disc contains the only photos of a deceased parent or a child's early years with no other copies, $200-300 for professional recovery may be an easy decision. If the disc duplicates photos that exist on other media, home attempts are worth trying first."
  - question: "How does NAFNet remove compression artifacts from recovered disc photos?"
    answer: "Photos recovered from Kodak Picture CDs, Fuji FinePix CDs, and similar consumer photo disc services were compressed as JPEG at moderate quality settings — typically quality 75-85 in JPEG terms. This compression introduces characteristic artifacts that become more visible as the images age or are re-processed: 8x8 pixel blocking in smooth gradient areas (sky, skin, backgrounds), ringing artifacts around sharp edges (tree branches, window frames, hair edges), and chroma subsampling artifacts in color transition areas. NAFNet addresses JPEG artifacts through a learned noise reduction model that distinguishes compression patterns from real image content. It identifies block boundaries and smooths them without blurring the genuine edges that define image content, and it suppresses ringing artifacts around edges while preserving edge sharpness. For recovered disc photos specifically, running NAFNet artifact removal before upscaling via Real-ESRGAN produces substantially better upscaled output — the upscaling model receives a clean input and extrapolates real image detail rather than amplifying compression blocks. The sequence at ArtImageHub is automatic in the Old Photo Restoration tool: the pipeline applies NAFNet denoising before Real-ESRGAN upscaling in a single upload. The $4.99 one-time access covers this complete pipeline."
---
