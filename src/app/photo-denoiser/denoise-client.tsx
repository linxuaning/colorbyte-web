...
export default function DenoiseClient() {
  ...
  // Check per-feature entitlement on mount
  useEffect(() => {
    if (!API_BASE) {
  ...
  // Restore result from resume_task_id
  useEffect(() => {
    if (!API_BASE || !resumeTaskId || stage !== "idle" || checkingAccess || !isEntitled) {
      return;
    }
    ...
    const poll = async () => {
      while (!cancelled) {
        try {
          ...
          setProgressText(data.stage || "Denoising...");
        ...
        resumeTaskId && checkingAccess ? (
          ...
          <div className="rounded-2xl border border-[#d2d2d7]/60 bg-[#f5f5f7] px-8 py-14 text-center">
            ...
            <h3 className="mt-5 text-[24px] font-semibold tracking-[-0.03em] text-[#1d1d1f]">
              Unlock Photo Denoiser Before Upload
            </h3>
            <p className="mx-auto mt-3 max-w-xl text-[14px] leading-[1.7] text-[#6e6e73]">
              Complete checkout to unlock the denoiser. Return with the same email to upload and keep paid download access tied to that purchase.
            </p>
            <Link
              href={checkoutHref}
            ...
            >
              ...
              Unlock Denoiser — {PRO_PRICE_TEXT}
            </Link>
            <p className="mt-3 text-[12px] text-[#6e6e73]">
              One-time payment · Lifetime access to the denoiser · Does not include other tools.
            </p>
            ...
            <div className="mx-auto mt-6 w-full max-w-xs rounded-xl border border-[#d2d2d7]/60 bg-white p-3 text-center">
              ...
              {paidCheckStatus === "notfound" && (
                <p className="mt-1.5 text-[11px] text-red-500">No denoiser access found for this email.</p>
              )}
            </div>
          </div>
        ) : (
          ...
          >
            ...
            <div>
              <p className="text-[17px] font-semibold text-[#1d1d1f]">Drop your noisy photo here</p>
            ...
            <p className="text-[12px] text-[#6e6e73]/80">
              Full resolution download — no watermark
            </p>
            <input
              ref={fileInputRef}
              type="file"
        ...
        <div className="flex flex-col items-center gap-7 rounded-2xl border border-[#d2d2d7]/50 bg-[#f5f5f7] p-12 text-center">
          ...
          <div className="w-full max-w-sm">
            ...
            <p className="mt-3 text-[14px] font-medium text-[#1d1d1f]">
              {progressText || "Denoising..."}{progress > 0 && ` — ${progress}%`}
            </p>
      ...
      {stage === "done" && resultPreviewUrl && (
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            {["AI Noise Removal", "NAFNet SIDD Model", "Detail Preserved"].map((tag) => (
          ...
          <div className="mx-auto max-w-md rounded-2xl border border-[#d2d2d7]/50 bg-[#f5f5f7] p-7">
              ...
              <div className="space-y-3">
                <div className="rounded-xl border border-[#0071e3]/15 bg-white p-4 text-left">
                  <p className="text-[14px] font-semibold text-[#1d1d1f]">
                    Your denoised photo is ready.
                  </p>
                  <p className="mt-1.5 text-[13px] leading-[1.6] text-[#6e6e73]">
                    Unlock once to download in original quality. The same email keeps access permanently.
                  </p>
                </div>
                <Link
                  href={(() => {
                  ...
                  onClick={() => trackCTAClick("denoiser-result-paywall")}
                ...
                >
                  <span className="flex items-center gap-2">
                    <Crown className="h-4 w-4" />
                    Unlock Denoiser to Download
                  </span>
                  <span className="text-[11px] font-normal opacity-70">
                    {PRO_PRICE_TEXT} one-time · Denoiser lifetime access
                  </span>
                </Link>
              </div>
            )}
            ...
            <div className="mt-3 flex items-center justify-center gap-1.5 text-[12px] text-amber-600">
              ...
              Your denoised photo will be deleted in 24 hours
            </div>
          </div>
      ...
      {stage === "idle" && (
        <div className="mt-16 border-t border-[#d2d2d7]/40 pt-12">
          ...
          <div className="grid gap-5 text-center sm:grid-cols-3">
            {[
              { n: "1", title: "Pay First", desc: "Unlock the denoiser with a one-time payment" },
              { n: "2", title: "Upload", desc: "Return with the same email and upload your noisy photo" },
...
function BeforeAfterSlider({
  ...
  return (
    ...
    >
      <img
        src={afterSrc}
        alt="Denoised photo after AI processing"
      ...
      >
        <img
          src={beforeSrc}
          alt="Original noisy photo before denoising"
          className="absolute inset-0 h-full w-full object-contain"
          loading="lazy"
        />
      </div>
