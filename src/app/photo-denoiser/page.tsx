...
const faqSchema = {
  ...
  mainEntity: [
    ...
    },
    {
      ...
      acceptedAnswer: {
        "@type": "Answer",
        text: "Most photos finish denoising in 30–60 seconds end-to-end, including upload, AI processing, and result rendering. Processing time scales with image dimensions: a typical 12-megapixel smartphone photo (4000x3000 pixels) takes about 30–60 seconds, a 24-megapixel DSLR photo takes 30–60 seconds, and a 50-megapixel medium-format scan can take up to 30–60 seconds. The NAFNet model runs the full image through the network in a single pass without tiling for images under about 25 megapixels, which avoids edge artifacts at tile boundaries. For very large images above 25 megapixels, the system uses overlapping tile processing that adds a few seconds of overhead. If you are processing a batch of denoising work, the per-image cost stays consistent — there is no warmup penalty after the first image, since the model stays loaded between requests for paid users. Processing time is the same for paid users.",
      },
    },
    {
      "@type": "Question",
          ...
          <dl className="space-y-6">
            {[
              ...
              },
              {
                ...
                a: "Most photos finish denoising in 30–60 seconds end-to-end, including upload, AI processing, and result rendering. Processing time scales with image dimensions: a typical 12-megapixel smartphone photo (4000×3000 pixels) takes about 30–60 seconds, a 24-megapixel DSLR photo takes 30–60 seconds, and a 50-megapixel medium-format scan can take up to 30–60 seconds. The NAFNet model runs the full image through the network in a single pass without tiling for images under about 25 megapixels, which avoids edge artifacts at tile boundaries. For very large images above 25 megapixels, the system uses overlapping tile processing that adds a few seconds of overhead. If you are processing a batch of denoising work, the per-image cost stays consistent — there is no warmup penalty after the first image, since the model stays loaded between requests for paid users. Processing time is the same for paid users.",
              },
              {
