...
export default function DeblurClient() {
    ...
    const poll = async () => {
      while (!cancelled) {
        try {
          ...
          setProgressText(data.stage || "Deblurring...");
  ...
  const handleAlreadyPaidCheck = useCallback(() => {
    ...
    fetch(
      `${API_BASE}/api/payment/feature-entitlement/${encodeURIComponent(paidEmail.trim().toLowerCase())}/${FEATURE_KEY}`
    )
      .then((r) => r.json())
      .then((data) => {
        if (data.is_entitled) {
    ...
    <div className="mt-10">
      {stage === "idle" && (
        resumeTaskId && checkingAccess ? (
          <div className="flex flex-col items-center gap-4 rounded-2xl border border-[#d2d2d7]/60 bg-[#f5f5f7] px-8 py-16 text-center">
            ...
            <p className="text-[13px] text-[#6e6e73]">Checking the email linked to this result.</p>
          </div>
        ) : !canUpload ? (
          <div className="rounded-2xl border border-[#d2d2d7]/60 bg-[#f5f5f7] px-8 py-14 text-center">
            ...
            <h3 className="mt-5 text-[24px] font-semibold tracking-[-0.03em] text-[#1d1d1f]">
              Unlock Photo Deblurrer Before Upload
            </h3>
            <p className="mx-auto mt-3 max-w-xl text-[14px] leading-[1.7] text-[#6e6e73]">
              Complete checkout to unlock the deblurrer. Return with the same email to upload and fix blurry photos instantly.
            </p>
            <Link
              href={checkoutHref}
            ...
            >
              ...
              Unlock Deblurrer — {PRO_PRICE_TEXT}
            </Link>
            <p className="mt-3 text-[12px] text-[#6e6e73]">
              One-time payment · Lifetime access · Does not include other tools.
            </p>
            ...
            <div className="mx-auto mt-6 w-full max-w-xs rounded-xl border border-[#d2d2d7]/60 bg-white p-3 text-center">
              ...
              {paidCheckStatus === "found" && (
                <p className="mt-1.5 text-[11px] text-green-600">Access restored.</p>
              )}
              {paidCheckStatus === "notfound" && (
                <p className="mt-1.5 text-[11px] text-red-500">No deblurrer access found for this email.</p>
              )}
            </div>
          </div>
        ) : (
          ...
          >
            ...
            <div>
              <p className="text-[17px] font-semibold text-[#1d1d1f]">Drop your blurry photo here</p>
              <p className="mt-1.5 text-[14px] text-[#6e6e73]">
                JPG, PNG, or WEBP · up to 20MB · drag, click, or paste
              </p>
            </div>
            <button
      ...
      {(stage === "uploading" || stage === "processing") && (
        <div className="flex flex-col items-center gap-7 rounded-2xl border border-[#d2d2d7]/50 bg-[#f5f5f7] p-12 text-center">
          {preview && (
            <img src={preview} alt="Photo being deblurred" className="h-48 w-auto rounded-xl object-contain shadow-sm" loading="lazy" />
          )}
          <div className="w-full max-w-sm">
            ...
            <p className="mt-3 text-[14px] font-medium text-[#1d1d1f]">
              {progressText || "Deblurring..."}{progress > 0 && ` — ${progress}%`}
            </p>
            {stage === "processing" && progress < 30 && (
              <p className="mt-1.5 text-[12px] text-[#6e6e73]">
                NAFNet is warming up — may take 30–60 seconds on first run.
              </p>
            )}
          </div>
      ...
      {stage === "done" && resultPreviewUrl && (
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            {["AI Deblurring", "NAFNet GoPro Model", "Sharpness Restored"].map((tag) => (
          ...
          <div className="mx-auto max-w-md rounded-2xl border border-[#d2d2d7]/50 bg-[#f5f5f7] p-7">
            ...
            {isEntitled ? (
              ...
              >
                <span className="flex items-center gap-2"><Crown className="h-4 w-4" />Download Sharp Photo</span>
              ...
              <div className="space-y-3">
                <div className="rounded-xl border border-[#0071e3]/15 bg-white p-4 text-left">
                  <p className="text-[14px] font-semibold text-[#1d1d1f]">Your sharp photo is ready.</p>
                ...
                <Link
                  ...
                  onClick={() => trackCTAClick("deblurrer-result-paywall")}
                ...
                >
                  <span className="flex items-center gap-2"><Crown className="h-4 w-4" />Unlock Deblurrer to Download</span>
                  <span className="text-[11px] font-normal opacity-70">{PRO_PRICE_TEXT} one-time · Deblurrer lifetime access</span>
                </Link>
              </div>
            )}
      ...
      {stage === "idle" && (
        <div className="mt-16 border-t border-[#d2d2d7]/40 pt-12">
          ...
          <div className="grid gap-5 text-center sm:grid-cols-3">
            {[
              { n: "1", title: "Pay First", desc: "Unlock the deblurrer with a one-time payment" },
              { n: "2", title: "Upload", desc: "Return with the same email and upload your blurry photo" },
              { n: "3", title: "Download", desc: "Download the sharp result in full resolution" },
            ].map((s) => (
...
function BeforeAfterSlider({ beforeSrc, afterSrc }: { beforeSrc: string; afterSrc: string }) {
  ...
  return (
    ...
    >
      <img src={afterSrc} alt="Sharp photo after AI deblurring" className="absolute inset-0 h-full w-full object-contain" loading="lazy" />
      <div className="absolute inset-0 overflow-hidden" style={{ width: `${sliderPos}%` }}>
        <img src={beforeSrc} alt="Blurry photo before deblurring" className="absolute inset-0 h-full w-full object-contain" loading="lazy" />
      </div>
