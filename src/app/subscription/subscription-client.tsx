...
export default function SubscriptionClient({
  ...
  const [clientResumeTaskId, setClientResumeTaskId] = useState(resumeTaskId);
  const [clientFunnelSource, setClientFunnelSource] = useState(funnelSource);
  const [clientFeatureKey, setClientFeatureKey] = useState(featureKey);
  ...
  useEffect(() => {
    ...
    const emailToCheck = initialEmail || saved;
    if (emailToCheck) {
      ...
      void (async () => {
        ...
        setError("");
        try {
          const data = await fetchSubscriptionStatus(emailToCheck);
          setSub(data);
          localStorage.setItem("artimagehub_email", emailToCheck);
        } catch {
          setSub(null);
  ...
  useEffect(() => {
    ...
    setClientResumeTaskId(params.get("resume_task_id")?.trim() || "");
    setClientFeatureKey(params.get("feature_key")?.trim() || "restoration");
    setClientFunnelSource(readPaymentFunnelSource(params));
  }, []);
        ...
        <div id="checkout-offer" className="mb-12 scroll-mt-24 flex flex-col">
          ...
          <div className="order-1 sm:order-2 max-w-md mx-auto w-full">
            ...
            <div className="relative rounded-2xl bg-[#1d1d1f] p-6">
              ...
              <DodoCheckoutButton
                checkoutEmail={email}
                resumeTaskId={clientResumeTaskId || undefined}
                funnelSource={clientFunnelSource}
